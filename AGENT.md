...
|---|---|---|---|
| 2026-09-24: Wallet focus lanes mirrored from ChurnPilot: `engines/walletFocus.js` (copied verbatim), `shared/WalletLanes.jsx` sections (Act now / Working / Waiting / Decision due + collapsed Earned & parked) on Recommended sort, `shared/FocusStrip.jsx` line + lane-colored left edge on CardItem/AccountItem, lane CSS in index.css. Earlier 2026-09-18: annual-fee refund on cancel/downgrade (`feeRefundAmount`, 0 = no refund; downgrade/Mark Closed now set `closedDate`; 'Did the annual fee come back?' check-back + Fee refunded / No refund buttons on retired cards); Tax page moved to the bottom of Earnings (/tax redirects); Trailing-12 stat → 'Lifetime Net After Tax'. Mirrored from ChurnPilot (engines copied). Earlier 2026-09-17: bank accounts 'Recommended' sort (default, `getAccountAttentionScore` in engines/lifecycle.js); add-account form shows monthly fee + waiver fields; Money Map 'It landed' also clears `check_dd` check-backs. Mirrored from ChurnPilot. Earlier: card DD deadline hidden once bonus paid or all DDs logged (was showing OVERDUE on Capital One); synced ChurnPilot's tabbed compact editor (`shared/CompactEditor.jsx` + css, bank/issuer dropdowns) into AccountItem + CardItem; two-app rule added. Earlier: fee waiver by debit swipes | `src/engines/walletFocus.js`, `src/components/shared/WalletLanes.jsx`, `src/components/shared/FocusStrip.jsx` | Visual check (needs a GitHub token to open the app) | Fee cycle day unknown for WF/Republic (month end assumed) |
