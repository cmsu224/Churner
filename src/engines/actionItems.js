...
export function generateActionItems(state) {
  ...
  for (const card of (state.creditCards ?? [])) {
    ...
    if (si && !si.met) {
      ...
      if (d < 0) {
        items.push({ id: `spend-overdue-${card.id}`, type: 'critical', category: 'spend', cardId: card.id, memberId: card.memberId,
        ...
        items.push({ id: `spend-urgent-${card.id}`, type: 'critical', category: 'spend', cardId: card.id, memberId: card.memberId,
        ...
        items.push({ id: `spend-warn-${card.id}`, type: 'warning', category: 'spend', cardId: card.id, memberId: card.memberId,
    ...
    if (card.bonusReceived && card.status === 'Active Churn') {
      items.push({ id: `stale-status-${card.id}`, type: 'info', category: 'bonus', cardId: card.id, memberId: card.memberId,
    ...
    if (ri && ri.reeligible) {
      items.push({ id: `reeligible-${card.id}`, type: 'info', category: 'reeligible', cardId: card.id, memberId: card.memberId,
  ...
  for (const acct of (state.bankAccounts ?? [])) {
    ...
    if (!acct.ddLinkedDate && !bonusReceived) {
      if (acct.openedDate && (acct.ddDeadlineDays ?? 0) > 0) {
        ...
        if (daysLeft < 0) {
          ...
          items.push({ id: `dd-urgent-${acct.id}`, type: 'critical', category: 'bonus', accountId: acct.id, memberId: acct.memberId,
          ...
          items.push({ id: `dd-warn-${acct.id}`, type: 'warning', category: 'bonus', accountId: acct.id, memberId: acct.memberId,
          ...
          items.push({ id: `dd-info-${acct.id}`, type: 'info', category: 'bonus', accountId: acct.id, memberId: acct.memberId,
    ...
    if ((acct.requiredDDCount ?? 1) > 1 && !bonusReceived) {
      ...
      if (remaining > 0 && made > 0) {
        items.push({ id: `dd-progress-${acct.id}`, type: 'info', category: 'bonus', accountId: acct.id, memberId: acct.memberId,
    ...
    if ((acct.minimumBalance ?? 0) > 0 && !bonusReceived) {
      items.push({ id: `min-bal-${acct.id}`, type: 'info', category: 'bonus', accountId: acct.id, memberId: acct.memberId,
    ...
    if (acct.openedDate && (acct.bonusDeadlineDays ?? 0) > 0 && !bonusReceived) {
      ...
      if (daysLeft < 0) {
        ...
        items.push({ id: `bonus-deadline-${acct.id}`, type: 'warning', category: 'bonus', accountId: acct.id, memberId: acct.memberId,
  ...
  for (const member of members) {
    const memberCards = (state.creditCards ?? []).filter(c => c.memberId === member.id)
    const keepAlive = getKeepAliveCards(memberCards)
    for (const { card, age, daysSinceUsed, usageStatus, isOldest } of keepAlive) {
      ...
      if (usageStatus === 'critical') {
        items.push({ id: `keepalive-crit-${card.id}`, type: 'critical', category: 'keepalive', cardId: card.id, memberId: member.id,
        ...
        items.push({ id: `keepalive-warn-${card.id}`, type: isOldest ? 'warning' : 'info', category: 'keepalive', cardId: card.id, memberId: member.id,
        ...
        items.push({ id: `keepalive-track-${card.id}`, type: 'info', category: 'keepalive', cardId: card.id, memberId: member.id,
