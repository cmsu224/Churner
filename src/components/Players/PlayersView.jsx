...
function PlayerForm({ initial, onSave, onCancel }) {
  ...
  return (
    <div className="bg-raised border border-edge-strong rounded-xl p-4 space-y-3">
      <div>
        <label className="text-xs text-ink-muted block mb-1">Name <span className="text-danger-ink">*</span></label>
        <input
          className={inp}
          value={name}
            ...
            <button
              ...
              className={`p-2 rounded-lg border text-left transition-colors ${
                role === r.value
                  ...
                  : 'border-edge-strong bg-surface text-ink-muted hover:border-edge-strong'
              }`}
            >
              <div className="text-xs font-medium">{r.label}</div>
              <div className="text-xs text-ink-tertiary mt-0.5">{r.desc}</div>
            </button>
          ))}
        </div>
      </div>
      ...
      <div>
        <label className="text-xs text-ink-muted block mb-1">Color</label>
        <div className="flex gap-2 flex-wrap">
          {COLORS.map(c => (
            <button
              ...
              className={`w-7 h-7 rounded-full transition-transform ${hex === c.hex ? 'scale-125 ring-2 ring-ink ring-offset-2 ring-offset-raised' : 'hover:scale-110'}`}
      ...
      <div className="flex gap-2 pt-1">
        <button type="button" onClick={onCancel} className="flex-1 bg-overlay hover:bg-overlay text-ink-secondary py-2 rounded-lg text-sm transition-colors">
          Cancel
        </button>
        <button
          type="button"
  ...
  return (
    <div className="p-4 max-w-2xl mx-auto space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-bold text-ink">Household Members</h1>
        {!adding && (
          <button
      ...
      <div className="space-y-3">
        {players.map(player => {
          ...
          if (confirmingId === player.id) {
            return (
              <div key={player.id} className="bg-surface border border-danger/30 rounded-xl p-4">
                <p className="text-sm text-ink-secondary mb-1">
                  Delete <strong className="text-ink">{player.name}</strong>?
                </p>
                ...
                <div className="flex gap-2">
                  <button onClick={() => setConfirmingId(null)} className="flex-1 bg-raised hover:bg-overlay text-ink-secondary py-2 rounded-lg text-sm transition-colors">Cancel</button>
                  <button
          ...
          return (
            <div key={player.id} className="bg-surface border border-edge-strong rounded-xl p-4 flex items-center gap-4">
              <span className="w-10 h-10 rounded-full flex-shrink-0 flex items-center justify-center text-sm font-bold text-ink" style={{ backgroundColor: player.hex }}>
                {player.name.charAt(0).toUpperCase()}
              </span>
              <div className="flex-1 min-w-0">
                <div className="font-semibold text-ink text-sm">{player.name}</div>
                <div className="text-xs text-ink-tertiary capitalize">{player.role} · {cardCount} card{cardCount !== 1 ? 's' : ''} · {accountCount} account{accountCount !== 1 ? 's' : ''}</div>
              </div>
              <div className="flex items-center gap-1">
                <button
                  onClick={() => setEditingId(player.id)}
                  className="p-2 text-ink-tertiary hover:text-ink transition-colors rounded-lg hover:bg-raised"
                >
                  <Pencil size={14} />
                </button>
                <button
                  ...
                  disabled={players.length <= 1}
                  className="p-2 text-ink-tertiary hover:text-danger-ink disabled:opacity-30 transition-colors rounded-lg hover:bg-raised"
                >
                  <Trash2 size={14} />
                </button>
              </div>
      ...
      {players.length === 0 && !adding && (
        <div className="text-center py-12 text-ink-tertiary">
          ...
          <button onClick={() => setAdding(true)} className="mt-3 text-accent-ink text-sm hover:underline">Add your first member</button>
        </div>
      )}
    </div>
  )
