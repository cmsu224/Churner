...
const USAGE_STYLE = {
  ok: { icon: CheckCircle, color: 'text-success-ink', label: 'Active' },
  warning: { icon: AlertTriangle, color: 'text-warning-ink', label: 'Use soon' },
  critical: { icon: AlertTriangle, color: 'text-danger-ink', label: 'At risk' },
  unknown: { icon: HelpCircle, color: 'text-ink-tertiary', label: 'No usage date' },
}
...
function CardRow({ card, age, daysSinceUsed, usageStatus, isOldest }) {
  ...
  return (
    <div className="flex items-center gap-2 bg-raised/60 rounded-lg px-3 py-2">
      ...
      <div className="flex-1 min-w-0">
        <div className="text-xs font-medium text-ink truncate flex items-center gap-1.5">
          {isOldest && <Star size={10} className="text-warning-ink fill-warning flex-shrink-0" />}
      ...
      <div className="flex items-center gap-2 flex-shrink-0">
        ...
        {daysSinceUsed !== 0 && (
          <button
            ...
            className="flex items-center gap-0.5 bg-raised hover:bg-success text-ink-muted hover:text-white text-xs px-1.5 py-0.5 rounded transition-colors"
          >
            <Zap size={10} />
          </button>
        )}
    ...
    <div className="bg-surface border border-edge-strong rounded-xl p-4">
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-1.5 text-xs text-ink-muted">
          <Clock size={12} />
          <span>Avg age <span className="text-ink font-medium">{stats.aaoaLabel}</span></span>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between mb-2">
        ...
        {atRisk > 0 && (
          <span className="text-xs font-medium text-warning-ink">{atRisk} need{atRisk === 1 ? 's' : ''} attention</span>
        )}
      </div>
      ...
      {tracked.length > COLLAPSED_LIMIT && (
        <button
          ...
          className="w-full mt-2 flex items-center justify-center gap-1 text-xs text-ink-muted hover:text-ink transition-colors py-1.5"
        >
          {showAll
      ...
      <div className="flex items-center gap-1.5 mt-2 pt-2 border-t border-edge text-xs text-ink-faint">
        <Star size={9} className="text-warning-ink fill-warning" />
...
export default function CreditAgeSection() {
  ...
  return (
    <section>
      <div className="flex items-center justify-between mb-3">
        <h2 className="text-base font-semibold text-ink">Credit Age & Card Usage</h2>
        <span className="text-xs text-ink-tertiary">Protects ~15% of your score</span>
      </div>
