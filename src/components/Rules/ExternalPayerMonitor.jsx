...
export default function ExternalPayerMonitor() {
  ...
  const inp = 'w-full bg-raised border border-edge-strong rounded-lg px-3 py-2 text-sm text-ink focus:outline-none focus:border-accent transition-colors'

  return (
    <div className="bg-surface border border-edge-strong rounded-xl p-5">
      <div className="flex items-center justify-between mb-1">
        <h3 className="text-base font-semibold text-ink">External Payer Monitor</h3>
        <button
          onClick={() => setAdding(true)}
          className="flex items-center gap-1.5 bg-raised hover:bg-overlay text-ink-secondary text-xs font-medium px-3 py-1.5 rounded-lg transition-colors"
        >
          <Plus size={12} />Add
        </button>
      </div>
      ...
      {(state.externalPayments ?? []).length === 0 ? (
        <div className="text-center py-6 text-ink-tertiary text-sm">No external payments tracked.</div>
      ) : (
            ...
            return (
              <div
                ...
                className={`flex items-start justify-between gap-3 p-3 rounded-lg border ${
                  !pmt.usePortal ? 'border-danger/30 bg-danger/5' : 'border-edge-strong bg-raised/50'
                }`}
              >
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap mb-0.5">
                    <span className="text-sm text-ink font-medium">{card?.cardName ?? 'Unknown card'}</span>
                  ...
                  <div className={`flex items-center gap-1 mt-1 text-xs ${pmt.usePortal ? 'text-success-ink' : 'text-danger-ink font-semibold'}`}>
                ...
                <button
                  ...
                  className="text-ink-tertiary hover:text-danger-ink p-1 transition-colors flex-shrink-0"
                >
                  <Trash2 size={13} />
                </button>
              </div>
      ...
      {adding && (
        <Modal title="Add External Payment" onClose={() => setAdding(false)}>
          <div className="space-y-3">
            <div>
              <label className="text-xs text-ink-secondary block mb-1">Senior&apos;s Card Being Paid</label>
            ...
            <div>
              <label className="text-xs text-ink-secondary block mb-1">Payer (Churner)</label>
            ...
            <div>
              <label className="text-xs text-ink-secondary block mb-1">Payer&apos;s Account Last 4</label>
              <input
                className={inp}
