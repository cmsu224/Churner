...
- **Responsive:** the columns switch to a compact width below 640px so the whole map fits a phone without scrolling sideways, and the per-ribbon amount labels step aside there (the gutter is narrower than the label, and each card already carries its own `+$X` badge). The footer hint matches the input you actually have — *tap a bank* on touch, *double-click / drag and drop* with a pointer. Reduced-motion users get the dashed ribbons without the marching.
