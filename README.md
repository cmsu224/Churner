...
- [Feature Catalog](#feature-catalog)
  ...
  - [7. Bank Account Tracking](#7-bank-account-tracking)
  - [8. Money Map: Transfers, Cash Position & Check-Backs](#8-money-map-transfers-cash-position--check-backs)
  ...
  - [12. Annual Fee Tracker](#12-annual-fee-tracker)
  ...
  - [20. Data Sync (Private GitHub Repository or Gist)](#20-data-sync-private-github-repository-or-gist)
  - [21. UI / UX & Design System](#21-ui--ux--design-system)
  - [22. Installable App (PWA) & Offline Support](#22-installable-app-pwa--offline-support)
  - [23. Milestone Tracker Table](#23-milestone-tracker-table)
...
- **Reapply for a bonus** — fires once a **closed** account's bank cooldown has cleared, so the bank should pay a new-account bonus again (see the [reapply clock](#7-bank-account-tracking)). Paired with a **reapply window opens in Nd** heads-up inside 30 days, so the direct-deposit source can be lined up before the window opens. One reminder per member + bank (the latest, binding cooldown — three closed Chase accounts produce one item, not three), and never while that person still holds an open account at the same bank.

**Monthly fee items (open bank accounts with a fee)**
- **Skip the monthly fee** — one item per account per fee cycle (`monthly-fee-<id>-<YYYY-MM>`, so next month's shows up again even if this one was dismissed) while the waiver isn't met: *info* until 3 days before the send-by date, *warning* inside that, *critical* in the last 3 days of the cycle (which also fires a browser notification). The detail spells out the exact step — add $X to reach the balance, and/or send the $Y direct deposit by the send-by date — and, during the clawback hold, how many days the account still has to stay open. Category **monthly fee**; links to the account.
...
**Money items (from the [Money Map](#8-money-map-transfers-cash-position--check-backs))**
- **A transfer that should have landed** (critical) — an ACH push still unconfirmed more than 5 days after it was sent. **A check-back you set** (warning) once it's due or overdue. **Cash with no reason to stay where it is** (info) — an account past its clawback window, or closed, still holding money 14+ days on. All three carry the **Money** category and link to `/money`, where marking it landed or pushing it home actually happens. Upcoming check-backs stay out of the queue until they're due — a reminder set three weeks out isn't an action item today — and a check-back that was only waiting on the money to arrive [leaves the queue the moment you mark the transfer landed](#8-money-map-transfers-cash-position--check-backs), rather than going overdue for a check you've already done.
...
**Tracked fields:** person, status, bank name, account type, last 4, opened date, **closed date**, **current balance**, bonus amount, bonus received date, **custom color / accent**, **direct-deposit requirements** (direct deposit amount, # of direct deposits required, # completed, direct deposit deadline in days, direct deposit linked date, direct deposit source — e.g. payroll, Social Security, ACH), **debit-card requirements** (# of debit purchases required, # completed, minimum per qualifying purchase, total debit spend required, debit spend logged, debit deadline in days, requirement-completed date), minimum balance, **monthly fee & how to avoid it** (fee, balance that waives it, monthly direct deposit that waives it, either/both rule, fee-cycle day), bonus deadline (days), **early-termination-fee window (days)**, taxable (1099-INT) flag, offer link, and notes.
...
- *Opened / Direct Deposit Linked* → **+ Debit Purchase N/M** whenever qualifying debit purchases are still outstanding (one tap per swipe, same as the direct-deposit counter). It sits behind the direct-deposit button while deposits are still owed — that's the step that needs money moved rather than a card tapped — and leads the row as the solid primary once they're done. The tap that clears the **last** outstanding requirement also stamps the completion date and moves the account to *Bonus Pending*.
- *Bonus Pending* → **✓ Bonus Posted** — sets the status, the received flag, **and the received date**, which is what puts the bonus in the right year on the [Tax page](#15-tax-liability-predictor) and starts the clawback countdown.
...
- *Any open account with a monthly fee* → **✓ Fee DD done (Mon)** marks this cycle's waiver deposit as made (tap again to undo). It hides when a Money Map direct-deposit push already covers the cycle, or when the balance alone waives the fee.
...
- a **count** of qualifying purchases (`requiredDebitCount` / `debitsMade`),
- a **minimum per purchase** that decides which swipes qualify (`requiredDebitAmount`) — a rule, not a target, so it's carried into the reminders rather than summed,
- a cumulative **debit spend** (`requiredDebitSpend` / `debitSpend`).

The **deadline** is its own field (`debitDeadlineDays`), but almost every offer runs the debit window on the same clock as the direct deposit, so an account that recorded only one window still gets a countdown: the engine falls back to the **direct-deposit window**, then the **overall bonus window** — and every readout names the window it borrowed, so a fallback never reads as a date you typed in. **Progress reports the binding half**: 9 of 10 swipes done with only half the required spend logged is 50% complete, not 90%. The engine feeds the debit action items, the debit-deadline event on the [Timeline](#4-timeline--calendar--ics-export), the **Debit** column in the [table view](#23-milestone-tracker-table), and the [Money Map](#8-money-map-transfers-cash-position--check-backs)'s per-account status line — so no page can claim the requirements are met while swipes are still owed.
...
**Where the money for the direct deposits comes from** is tracked separately, on the [Money Map](#8-money-map-transfers-cash-position--check-backs): every ACH push into an account, what's still in flight, what the account is holding, and when to sweep it home. An account's `currentBalance` is the field both pages share — landing a transfer credits it here, and it stays hand-editable on this page. A second field, **Started with** (`openingBalance`), records what the account held *before* its first logged push — $0 for a churn you opened empty, and whatever was already in there for an everyday account you added to the map later. It defaults to whatever balance a new account is created with, and it is kept as the record of where the balance started.

**Table view** — a **Table** toggle beside *Group by bank* swaps the account list for the dense milestone grid described in [Milestone Tracker Table](#23-milestone-tracker-table), carrying the opened → requirement → direct deposit → posted → close-after → closed → reapply chain across one row per account.

**Contextual edit form** — both the add and edit forms adapt to the account's status. The **Sign-Up Bonus** section (bonus amount, deadline, minimum balance, ETF window, taxable checkbox, bonus received date) only appears when the account is in a bonus-earning status or bonus data exists. The **Direct Deposit Requirements** section only appears when the status is Opened/DD Linked or when direct deposit data is present, and the **Debit Card Requirements** section follows the same rule against its own data (an offer with debit purchases and no direct deposit at all is common). The **Closed Date** field appears when the status is Closed or a date is already set. On the add form, current balance, minimum balance, bonus received date, offer link, and notes stay tucked under **More details (optional)**. "Direct Deposit" is always spelled out in full — never abbreviated.

### 8. Money Map: Transfers, Cash Position & Check-Backs
...
**On a phone the two columns and the gutter between them are sized to the panel** (down to a 124px floor), so the cards, their member badges and their in-flight pills all fit on screen — a fixed 152 + 72 + 152 layout hid 50px off the right edge of a 360px screen.

- **Node cards** are laid out **name-first**: the bank's **brand logo** (the same Google-favicon logo the rest of the app uses, falling back to a bank/wallet/house icon for a source the issuer table doesn't know) and the bank name get a full-width line of their own and wrap to two — three on a phone — so a long name reads in full instead of being clipped to `Bank o…`. Underneath sit the balance and, on their own line, the badges: the member/player attribution (`[● Me]`, `[● Partner]`) and `HUB`. The last line is the at-a-glance state, **derived from the account's own status** ([section 8](#7-bank-account-tracking)) so the map can never contradict the Accounts page — `Opened · no requirements set`, `Direct deposit linked`, `2 more direct deposits`, `7 more debit purchases`, `Requirements met · bonus pending`, `Bonus in · hold 43d`, `Bonus in · safe to close`, `Safe to close`, plus `Home base · money comes back here` for the hub and `Closed` — with a **`+$X` badge** for money in flight toward it, and on a phone a short form (`Opened`, `DD linked`, `3 more DDs`, `7 more debit`, `Hold 43d`, `Home base`) that fits the narrow card rather than truncating mid-word. The lifecycle stage decides *which* line shows; the direct-deposit counters, the debit-purchase counters and the 181-day clawback clock only fill in its numbers. Deposits lead the line while any are still owed and debit purchases take over once they're in, so `Requirements met` can never appear while either is outstanding. An account that is merely open, with no direct-deposit requirement recorded and none logged, says so plainly rather than claiming the bonus is being worked. A custom bank accent color, when set, paints the card's left edge. Accounts sort by urgency first, then by how much they're holding; the hub sits at the top of the left column.
- **Edit bank / source details directly from the map (`NodeEditModal.jsx`).** **Double-click** on any node card (or click its pencil edit icon, or tap **Edit details** in the mobile transfer sheet) to modify its name, balance, **Started with** ([`openingBalance`](#7-bank-account-tracking)), custom brand/accent color (from a palette of curated bank presets or custom hex code), account type, member attribution, status, hub flag, and whether it's **hidden from the map**. The dialog is headed by the bank's brand logo, and the `···last4` digits sit inline beside the bank name.
- **Interactive Transfer Flow on the Map.** Hovering a card reveals its shortcut row (**Send home** / **Transfer** / **Edit** / **hide**) below it — revealed on hover rather than pinned on, so the buttons can't sit over the top edge of the card underneath and swallow its clicks. Click **Transfer** on any account or source to enter interactive connect mode (with a glowing pulse and dynamic animated connection ribbon when hovering targets). Clicking any second bank or source opens the **Log Transfer modal** with pre-filled counterparties, quick amount chips, purpose pills (`Direct Deposit`, `Funding`, `Sweep Home`, `Other Push`), date selector, and reminder checkbox. **Send home** is a 1-click shortcut directly targeting the hub, and is hidden on the hub itself — money there is already home.
...
- **Hide any card you don't want on the map — from wherever you're looking at it.** The hub is the one card that can't be hidden, since everything aims money back at it (`hideBlockedReason`). Four entry points, all writing the same `hidden` flag in `moneyMapLayout`:
  - the **eye-off button** in a card's hover shortcut row, beside *Send home / Transfer / Edit*;
  - **Hide from the map** in the mobile transfer sheet (tap any card), which is the phone's route, since there is no hover there;
  - a **Hide from the Money Map** checkbox in the edit dialog, directly above Delete so the two choices are on one screen;
  - the eye-off button on a card in **Arrange Mode**.
- **Un-hiding is deliberately easy to find.** Hidden cards join the closed-and-empty ones behind a single **`N cards hidden — show`** toggle in the header — one drawer, so there's only ever one place to look for a card that isn't where you expect. Revealing draws them in place with a dashed amber border, and each one carries its own **Show** button (or **Show on the map** in the phone sheet); **Put it back on the map** next to the toggle un-hides all of them at once (`clearHiddenNodes`, which leaves the column arrangement intact). The edit dialog's checkbox un-ticks too.
  - **Hiding is display only, and never deletes anything.** A hidden account still holds its balance, still counts in every total, still appears on the Accounts page, and still raises its sweep-back reminder — so money can't go quiet just by leaving the picture. When something off the map is still holding cash, the header says so next to the toggle (*"still holding $2,500 — counted in the totals"*), because "hidden" must never start to feel like "gone".
  - **Delete is the other thing, and says so.** *Delete everywhere* in the edit dialog removes the account or source from the whole app — the Accounts page, its balance, its bonus tracking, its reminders — and its balance stops counting in the totals. Its confirm step spells that out, notes that already-logged transfers stay in the ledger behind a greyed-out *"Removed account"* placeholder, and offers **Hide from the map instead** for the case that's what was actually wanted.
...
1. **Direction** — *Send money out* (this account is the source), *Pull money in* (this account is the destination), *See its transfers* (filters the ledger and closes), *Edit details* (opens the edit dialog), or *Hide from the map* / *Show on the map* (the phone's hide toggle, absent on the hub). The sheet says outright that both directions record the same thing; which end you tapped only decides which side the account sits on.
...
**Bank reapply clock** (`bankReeligibility.js`) — the same rules applied at the **account** level instead of the bank level, for **closed accounts only**: the per-account second clock on the [Bank Accounts page](#7-bank-account-tracking). It calls `getBankRule` and `getBonusAnchor` from `bankEligibility.js` rather than restating any window or anchor rule, so a change to a bank's cooldown — or to what that cooldown counts from — moves both views at once.
...
- **Annual fee timing — predicted vs. confirmed** (`getAnnualFeeInfo`) — an annual fee does **not** post on the date the app can predict. Issuers bill it on the **first statement that closes on or after the cycle date** (the open date in year one, the anniversary after that), so it routinely lands weeks late — the sign-up fee on a card opened last week almost certainly hasn't posted yet. The engine therefore never *assumes* a posting; it predicts a window and waits for you to confirm the real date. Each fee cycle is in exactly one of three phases:
  ...
  - Confirming one posting also pins every later cycle to the card's true statement date, which is what makes year two onward exact. The engine also owns the **first-year-fee-waiver** skip, so the fee reminders, Timeline events, card verdicts, and [Annual Fee Tracker](#12-annual-fee-tracker) all honor it identically. Calendar dates are parsed at local midnight, so no countdown or displayed date shifts by a day in a negative-UTC timezone.
...
- **Earnings over time** — a hand-rolled SVG chart of the last 24 months, stacked by member (member identity colors, gridlines, month labels, per-segment tooltips, accessible label). Axis ticks keep one decimal below $100K, so a $1,350 month (axis max $2,000, ticks 1500/2000) doesn't label two gridlines "$2K".
...
- **Searches everything**: cards, bank accounts, points balances, members, and every page — grouped results with member/issuer context lines.
- **Selecting an item jumps to it** and pulses a highlight ring around it (`?highlight=` deep links).
- **Quick actions**: *Add card*, *Add application*, *Add bank account*, *Add points balance*, *Log a transfer* (opens the [Money Map](#8-money-map-transfers-cash-position--check-backs) quick bar), *Export calendar (.ics)*, and *Log spend on \<card\>* for every card with an open spend requirement.
...
- **AI Import Helper** — the fastest way to bulk-load. The prompt is **generated dynamically** with your actual household member names (e.g. Me | Wife | Mom | Dad) so the AI knows exactly who to assign each card to. Copy the prompt, open Claude (or any AI chat), paste the prompt + your credit-report PDF or screenshot, tell the AI whose cards you're importing ("These are Wife's cards" or "assign each to the right person"), and paste the returned JSON back into the app. The AI outputs a `member` field on each item; the import automatically resolves it to the correct member. Works for single-person and multi-person imports in one batch.
  ...
  - Manual/screenshot import: supports all fields including bonus details, spend requirements, DD requirements, debit-card requirements, annual fees, and status. A bank account imported as *Closed* carries its **closed date** through, so historical accounts land in the [reapply tracker](#7-bank-account-tracking) immediately.
...
### 20. Data Sync (Private GitHub Repository or Gist)
...
**One scroller.** The app shell is pinned to the viewport (`.app-shell`: `100dvh`, falling back to `100vh`, with `overflow: hidden`) and the content area — `<main>`, `flex-1 min-h-0 overflow-y-auto` — is the only thing that scrolls; the bottom nav sits fixed over it. `min-h-0` is what lets that flex child shrink below its own content height. Without the pin, `<main>` grew to fit its content and never overflowed, so it was an `overflow-y: auto` box that could not scroll standing between the finger and the document scroller that actually had somewhere to go — a touch drag hit-tests to the nearest scroll container, found that one, and did nothing. (Which is also why `overscroll-behavior` is only ever set on an element that really scrolls.)

**Safe areas.** The header reserves the status bar and the bottom nav reserves the home indicator, taking whichever is larger of `env(safe-area-inset-*)` and the `--inset-*` custom properties. A native shell that knows its own system-bar heights — Android's WebView reports only a display cutout through `env()`, never the bars — can publish them into those variables; in a browser both are absent and the plain fallbacks apply.

**Phone-first details.** Every numeric field declares `inputMode="decimal"`, so entering a dollar amount brings up the keypad rather than the letter keyboard. Moving between pages scrolls the content area back to the top (a deep link with `?highlight=` still scrolls to its own item), a path that matches no route lands on the Dashboard instead of a blank page, and the content area clears both the bottom nav and the home-indicator inset. Controls don't flash the grey tap highlight or raise the long-press selection bubble, and on a touch device every control gets a 44 × 44px minimum hit area. Fields stay at 16px on small screens, below which iOS zooms the page in on focus.

### 22. Installable App (PWA) & Offline Support
...
- **Bank account columns:** Bank (with logo, member dot, last 4) · Person · Type · Status · Opened · Bonus · Requirement · DD · Debit · Posted · Close after · Closed · Reapply.
  - **Requirement** is rebuilt from the account's structured fields — `$500 DD ×2 · 10 debit $5+ · $1,500 bal · 90d` — so the plan's shorthand is derived, never a second free-text field to keep in sync. A debit window only earns its own `90d debit` chip when it differs from the direct-deposit one it would otherwise borrow.
  ...
  - **Debit** shows the completed/required purchase count the `+ Debit Purchase N/M` button increments, or the running debit spend when the offer counts dollars instead of swipes — green once the whole requirement is met.
  - **Close after** is the [181-day clawback shield](#7-bank-account-tracking); **Reapply** is the [bank reapply clock](#10-issuer-rule-engines), reading *Now*, a date, *Lifetime*, or *Close others* when another account at that bank is still open.
- **Credit card columns:** Card (with logo, member dot, last 4) · Person · Status · Opened · Bonus · Min spend · Spent · Spend by · Bonus posted · Fee · Fee due · Close after · Closed.
  ...
  - **Fee due** carries the fee phase from the [lifecycle engine](#11-card-lifecycle-annual-fees-retention--re-eligibility) — a countdown, *any day* while a due fee waits on the statement, or the refund clock once you confirm it posted.
...
bankAccounts[]     { id, memberId, status, bankName, accountType, last4,
                     ...
                     requiredDebitCount, debitsMade,   ← debit-card purchases
                     requiredDebitAmount,  ← minimum size of a qualifying purchase
                     requiredDebitSpend, debitSpend,
                     debitDeadlineDays,    ← falls back to the DD / bonus window
                     debitCompletedDate,
...
reminders[]        { id, kind: 'check_bonus'|'check_transfer'|'sweep_back'
                     ...
                     createdAt }
applications[]     ← legacy. The Application Tracker page is gone; nothing
                     writes this any more. Existing records keep it, and it is
                     loaded, synced and exported untouched, so removing the
                     page never deletes anyone's application history.
...
Engines with no synced state of their own: `events.js` (timeline), `annualFees.js` (fee tracker), `earnings.js`, `burnRate.js`, `moneyFlow.js` (the money map's nodes, ribbons, totals and quick-entry parser — all derived from `cashSources`, `transfers` and the accounts' own balances), `reminders.js` (merges the stored `reminders[]` with derived late-transfer and stranded-cash rows, which are deliberately never persisted), `bankReeligibility.js` (bank reapply clock — derived entirely from account dates + `bankEligibility.js`'s windows and basis), `monthlyFee.js` (monthly fee waiver status and per-cycle reminders — derived from the account's fee fields, balance, `feeWaiverDDLog` and landed DD transfers), `debitCard.js` (debit-purchase requirement — derived entirely from the account's own counters and its debit / direct-deposit / bonus windows), `chexSystems.js` (inquiry counts — derived entirely from `bankAccounts[].openedDate` plus each bank's `chex` classification).
