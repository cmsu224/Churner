...
- **Edit bank / source details directly from the map (`NodeEditModal.jsx`).** **Double-click** on any node card (or click its pencil edit icon, or tap **Edit details** in the mobile transfer sheet) to modify its name, balance, **Started with** ([`openingBalance`](#8-bank-account-tracking)), custom brand/accent color (from a palette of curated bank presets or custom hex code), account type, member attribution, status, hub flag, and whether it's **hidden from the map**. The dialog is headed by the bank's brand logo, and the `···last4` digits sit inline beside the bank name.
