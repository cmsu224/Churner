...
- **Status-aware card display** — what the collapsed card shows depends on where it is in its lifecycle:
  ...
  - *Bonus Earned / Cancel or Downgrade* — the decision facts: **Bonus earned** (cash, or points + est. value), **Next annual fee** (amount · date · day countdown — or, when a fee just posted, the **30-day refund deadline**), and the one-line **cancel-or-downgrade verdict** (next bullet), which carries the 12-month clawback / safe-to-cancel timing.
...
- **Cancel-or-downgrade guidance** (`src/engines/cancelGuidance.js`) — every card past the earning stage (Bonus Earned / Keep Alive / Cancel or Downgrade) gets a verdict that combines the close shield, the next fee posting, and the refund window: **Wait to cancel** (clawback risk, with the day countdown), **Decide now** (a fee just posted — cancel or downgrade within the refund window for a full refund), **Cancel or downgrade soon** (fee posts within 45 days), **No rush yet** (fee further out), or **Keep it open** (no annual fee — a free open card helps credit age). On the collapsed card it's a **single truncated line** (verdict + a few-word summary) with the **full reasoning in the hover tooltip**; the expanded view spells it out. A fee card with no open/fee-post date is told to add one instead.
