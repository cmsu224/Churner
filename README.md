...
- **Status-aware card display** — what the collapsed card shows depends on where it is in its lifecycle:
  ...
  - *Bonus Earned / Cancel or Downgrade* — the decision facts: **Bonus earned** (cash, or points + est. value), **Next annual fee** (amount · date · day countdown — or, when a fee just posted, the **30-day refund deadline**), **Safe to cancel** (the 12-month clawback-shield date), and the **cancel-or-downgrade verdict** (next bullet).
...
- **Cancel-or-downgrade guidance** (`src/engines/cancelGuidance.js`) — every card past the earning stage (Bonus Earned / Keep Alive / Cancel or Downgrade) gets a one-line verdict that combines the close shield, the next fee posting, and the refund window: **Wait to cancel** (clawback risk, with the date it clears), **Decide now** (a fee just posted — cancel or downgrade within the refund window for a full refund), **Cancel or downgrade soon** (fee posts within 45 days), **No rush yet** (fee further out — plan to downgrade to keep the credit line and account age, or cancel before it posts), or **Keep it open** (no annual fee — a free open card helps credit age and utilization). A fee card with no open/fee-post date is told to add one instead.
...
- **Safe-to-cancel shield** — Bonus Earned and Cancel-or-Downgrade cards show the 12-month close shield as a **Safe to cancel** fact row: an amber date + day countdown until 1 year from open, green **"now — clawback window passed"** after. A card in *Bonus Earned* status counts as earned **even when the "bonus received" checkbox wasn't ticked** (the same rule the pipeline uses). Closing earlier risks the issuer clawing back the sign-up bonus — this is the card version of the bank accounts' 181-day clawback shield.
...
- **Per-person filter** buttons plus a full **filter + sort bar** on both the Credit Cards and Bank Accounts pages:
  - **Credit cards** — filter by status (multi-select), issuer/brand (auto-populated, multi-select), age range (< 1yr / 1–2yr / 2–4yr / 4+yr), and toggle chips for "Has annual fee", "Bonus pending", "Hide closed". Sort by Newest, Oldest, Highest annual fee, or Name A–Z.
...
- **12-month close shield** (`getCardCloseShield`) — the card version of the bank 181-day clawback rule: a card whose bonus has been earned becomes safe to close **365 days after opening** (closing earlier risks a bonus clawback). A card in *Bonus Earned* status counts as earned even without the received checkbox ticked — the same rule the pipeline uses. Powers the Safe-to-cancel fact row on each card, the primary quick-action flip on Bonus Earned cards, the approaching/cleared reminders, and the Timeline's card safe-to-close events.
- **Cancel-or-downgrade guidance** (`src/engines/cancelGuidance.js`) — folds the close shield, the next fee posting (`getCardFeeSchedule`), and the 30-day refund window into one per-card verdict (`wait` / `act` / `decide` / `keep`) with a plain-English reason and the date it hinges on. Only cards past the earning stage get a verdict — while a bonus is being earned, the spend tracker owns the advice.
...
- **Realized value per card**: bonus counted once received — cash bonuses at face value, points/miles at the card's **global program rate** (inferred from the card name/issuer, then valued at the per-program Settings rate or the published default, falling back to the household fallback ¢/point rate for unknown programs; estimated values are flagged `est.`; there is no per-card rate override). **Fees paid** counts only fee postings that have **actually occurred**, on the card's **fee-anchor cycle** — the recorded Annual Fee Post Date when set (statement fee dates often lag the open date), otherwise the open date, where the opening-day posting is the year-1 fee. First-year-waived skips the first posting; the closed date stops the clock; and a posting the card was **closed within 30 days after is treated as refunded** (the same 30-day cancel-for-refund rule the Annual Fee tracker uses), so it isn't counted. **Net = realized − fees.**
